...
import { useGetBrandQuery } from "@/redux/feature/api/brand/BrandApi";
...
const BrandList = () => {
  ...
  const { data, isLoading, isError, error } = useGetBrandQuery(currentPage);
  ...
  return (
    <div>
      ...
      <div className="mt-5 mx-5  flex items-center justify-between flex-col md:flex-row">
        ...
        <div>
          <Pagination paginationKey="brandList" />
        </div>
      </div>
    </div>
  );
