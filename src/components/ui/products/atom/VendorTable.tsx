...
import { useGetVendorTableQuery } from "@/redux/feature/api/product-list/VendorTableApi";
...
const VendorTable= () => {
  ...
  const key = "vendorTable";
  const dispatch = useAppDispatch();
  const currentPage = useAppSelector(
    (state) => state.pagination[key]?.currentPage
  );

  const { data, isLoading, isError } = useGetVendorTableQuery(currentPage);
  ...
  return (
    <div>
      ...
      <div className="mt-5 flex items-center justify-between flex-col md:flex-row">
        ...
        <div>
          <Pagination paginationKey="vendorTable" />
        </div>
      </div>
    </div>
  );
