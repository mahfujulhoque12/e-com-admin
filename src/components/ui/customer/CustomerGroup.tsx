...
const CustomerGroup = () => {
  ...
  const { data, isLoading, isError, error } =
    useGetGroupCustomerQuery(currentPage);
  ...
  return (
    <div>
      <div className="mt-5 bg-primary p-5 rounded-md shadow-md">
        <h2 className="text-color text-xl font-semibold">Customer Group</h2>
            ...
            <tbody className="bg-primary">
              {products.map((product) => (
                ...
                >
                  ...
                  <td
                    onClick={() => handleToggleText(product.id)}
                    className="px-6 py-4 whitespace-nowrapont-medium text-sm text-[#455468] dark:text-gray-300 cursor-pointer"
                  >
                    {showFullText === product.id
                      ? product.groupDes
                      : `${product.groupDes.slice(0, 30)}...`}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrapont-medium text-sm text-[#455468] dark:text-gray-300">
                    {product.totalCustomer}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
      ...
      <div className="mt-5 mx-5 flex items-center justify-between flex-col md:flex-row">
        ...
        <div>
          <Pagination paginationKey="groupCustomer" />
        </div>
      </div>
    </div>
  );
