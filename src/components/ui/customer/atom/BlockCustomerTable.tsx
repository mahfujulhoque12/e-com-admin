...
import { useGetblockCustomerQuery } from "@/redux/feature/api/customer-list/BlockedCustomerApi";
...
const BlockCustomerTable = () => {
  ...
  const { data, isLoading, isError, error } =
    useGetblockCustomerQuery(currentPage);
  ...
  return (
    <div>
      ...
      <div className="mt-5 flex items-center justify-between flex-col md:flex-row">
        ...
        <div>
          <Pagination paginationKey="blockCustomer" />
        </div>
      </div>
    </div>
  );
...
export default BlockCustomerTable;
